...
function setupVersionDateButton() {
  const button = document.getElementById("show-version-date");
  const output = document.getElementById("version-date-output");
  if (!button || !output) return;

  button.addEventListener("click", () => {
    const scriptUrl = document.querySelector('script[src*="app.js"]')?.getAttribute("src") || "app.js";
    const scriptVersion = new URL(scriptUrl, window.location.href).searchParams.get("v");
    const modified = new Date(document.lastModified);
    const modifiedText = Number.isNaN(modified.getTime())
      ? "Fecha no disponible"
      : modified.toLocaleString("es-CL");

    output.textContent = scriptVersion
      ? `Última versión desplegada: ${modifiedText} (build ${scriptVersion.slice(0, 8)}).`
      : `Última versión visible en este navegador: ${modifiedText}.`;
  });
}
